<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Timetable') }}: {{ $timetable->classroom->classroom_name }}
        </h2>
    </x-slot>

    <div class="max-w-7xl mx-auto mt-6 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <form action="{{ route('timetables.update', $timetable->id) }}" method="post" class="p-6 bg-white border-b border-gray-200"> @csrf @method('PUT')
                @forelse ($timetable->classroom->students as $student)                    
                    <div class="mb-4">
                        <label class="form-label inline-block mb-2 text-gray-700">{{ $student->user->name }}</label>
                        <div class="flex">
