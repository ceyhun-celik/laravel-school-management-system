                    ...
                    <div class="overflow-hidden">
                        @can('principal')
                            <div class="flex justify-end mb-4">
                                <a href="{{ route('timetables.create') }}" class="inline-block px-6 py-2.5 mb-1 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Create</a>
                            </div>
                        @endcan
                        ...
                        <table class="min-w-full">
                            <thead class="bg-white border-b">
                                <tr>
                                    ...
                                    <th scope="col" class="w-1 text-sm font-medium text-gray-900 px-6 py-4 text-left whitespace-nowrap">Classroon Name</th>
                            ...
                            <tbody>
                                @forelse ($timetables as $timetable)
                                    <tr class="{{ $loop->odd ? 'bg-gray-100' : 'bg-white' }} border-b">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $timetable->id }}</td>
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $timetable->classroom->classroom_name }}</td>
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $timetable->teacher->user->name }}</td>
                                        <td>
                                            <div class="flex items-center justify-end gap-3 mr-3">
                                                ...
                                                @can('principal')
                                                    <form action="{{ route('timetables.destroy', $timetable->id) }}" method="post" onsubmit="return confirm('Are you sure?')"> @csrf @method('DELETE')
