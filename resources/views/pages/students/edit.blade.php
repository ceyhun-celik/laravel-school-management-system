<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Student') }}: <span class="underline">{{ $student->user->name }}</span>
        </h2>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto mt-6 sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            ...
            <form action="{{ route('students.update', $student->id) }}" method="post" class="p-6 bg-white border-b border-gray-200"> @csrf @method('PUT')
                <div class="mb-4">
                    ...
                    <select name="classroom_id" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" id="Classroom Name">
                        @forelse ($classrooms as $classroom)
                            ...
                            <option {{ old('classroom_id') ?? $student->classroom_id == $classroom->id ? 'selected' : '' }} value="{{ $classroom->id }}">{{ $classroom->classroom_name }}</option>
                        @empty
