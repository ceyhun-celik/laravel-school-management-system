                ...
                <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="overflow-hidden">
                        <div class="flex justify-end mb-4">
                            <a href="{{ route('teachers.create') }}" class="inline-block px-6 py-2.5 mb-1 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Create</a>
                        </div>
                            ...
                            <tbody>
                                @forelse ($teachers as $teacher)
                                    <tr class="{{ $loop->odd ? 'bg-gray-100' : 'bg-white' }} border-b">
                                        ...
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $teacher->user->name }}</td>
                                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">{{ $teacher->lesson->lesson_name }}</td>
                                        <td>
                                            <div class="flex items-center justify-end gap-3 mr-3">
                                                <a href="{{ route('teachers.edit', $teacher->id) }}" class="inline-block px-6 py-2.5 bg-purple-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-purple-700 hover:shadow-lg focus:bg-purple-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-purple-800 active:shadow-lg transition duration-150 ease-in-out">Edit</a>
                                                
                                                <form action="{{ route('teachers.destroy', $teacher->id) }}" method="post" onsubmit="return confirm('Are you sure?')"> @csrf @method('DELETE')
